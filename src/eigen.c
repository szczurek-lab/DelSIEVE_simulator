...
// added code
int EigenREVMD(double Root[], double Cijk[]) {
  int i, j, k, n = 15;
  double U[n * n], V[n * n], T1[n * n], T2[n * n];

  abyx(1 / mr, QijMD, n * n);

  if ((k = eigen(1, QijMD, n, Root, T1, U, V, T2)) != 0) {
    fprintf(stderr, "\ncomplex roots in EigenREV");
    exit(0);
  }
  xtoy(U, V, n * n);
...
int EigenREVMDI(double Root[], double Cijk[]) {
  int i, j, k, n = 35;
  double U[n * n], V[n * n], T1[n * n], T2[n * n];

  abyx(1 / mr, QijMDI, n * n);

  if ((k = eigen(1, QijMDI, n, Root, T1, U, V, T2)) != 0) {
    fprintf(stderr, "\ncomplex roots in EigenREV");
    exit(0);
  }
  xtoy(U, V, n * n);
...
int eigen(int job, double AA[], int n, double rr[], double ri[], double vr[],
  ...
  balance(AA, n, &low, &hi, work);
  elemhess(job, AA, n, low, hi, vr, vi, (int *)(work + n));
  if (-1 == realeig(job, AA, n, low, hi, rr, ri, vr, vi))
    return (-1);
  if (job)
    ...
    for (;;) {
      ...
      for (m = en - 2; m >= l; m--) {
        ...
        if (m == l || fabs(mat[pos(m, m - 1, n)]) * (fabs(q) + fabs(r)) <=
                          eps *
                              (fabs(mat[pos(m - 1, m - 1, n)]) + fabs(z) +
                               fabs(mat[pos(m + 1, m + 1, n)])) *
                              fabs(p))
          break;
      }
      for (i = m + 2; i <= en; i++)
    ...
    for (en = n - 1; en >= 0; en--) {
      ...
      if ((q = vali[en]) < 0) { /* complex vector */
        ...
        if (fabs(mat[pos(en, en - 1, n)]) > fabs(mat[pos(en - 1, en, n)])) {
          ...
          mat[pos(en - 1, en, n)] =
              (p - mat[pos(en, en, n)]) / mat[pos(en, en - 1, n)];
        } else {
        ...
        for (i = en - 2; i >= 0; i--) {
          ...
          if (vali[i] < 0) {
            ...
            if (vali[i] == 0) {
              ...
              v = cdiv(compl(x * r - z * ra + q * sa, x * s - z * sa - q * ra),
                       v);
              mat[pos(i, en - 1, n)] = v.re;
              mat[pos(i, en, n)] = v.im;
