...
typedef struct {
  ...
  /* added code */
  int isMutated, isInserted, isDeleted;
  int numInsertions;
  int MLVariantNucleotide;
  int hasEventsOnLeaves;
  double allelicCov, allelicRawVar;
  /* end added code */
  int numCNLOH, numCNLOHmaternal, numCNLOHpaternal;
  int hasADO;
  int hasGenotypeError;
  int countA, countC, countG, countT, countN,
      countACGT; // alelle counts in genotypes
  int readCountA, readCountC, readCountG, readCountT,
      readCountACGT; // alelle counts in reads
